# coding=utf-8
""" generate_random.py for gitagovinda
"""
...
def set_pagerec_key_helper(rec,dictcode):
 if dictcode in ('pwg2','pw2','pwkvn2','sch2','mw2'):
  b = rec.adhy
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (b,fromv)
...
def get_dict_regex(dictcode):
 d = {
  # 2 numeric paramaters
  'pwg2':r'<ls>GĪT. ([0-9]+),([0-9]+)',
  'pw2':r'<ls>GĪT. ([0-9]+),([0-9]+)',
  'pwkvn2':r'<ls>GĪT. ([0-9]+),([0-9]+)',
  'sch2':r'<ls>Gīt. ([0-9]+),([0-9]+)',
  'mw2':r'<ls>Gīt. ([xiv]+), *([0-9]+)',
