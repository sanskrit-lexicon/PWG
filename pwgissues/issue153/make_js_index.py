# coding=utf-8
""" make_js_index.py for Gitagovinda
"""
...
parm_page = r'^([0-9]+)$'
parm_adhy = r'^([0-9]+)$'
parm_sarga = r'^(.*)$'   # unused in pwg references
...
""" 
 def __init__(self,line,iline):
  ...
  # set self.adhy as integer
  self.sarga = m_sarga.group(1)  # not used int(m_sarga.group(1))
 ...
 def todict(self):
  e = {
   'page':int(self.page),
   'adhy':int(self.adhy),
   # 'sarga':int(self.sarga), # not used in pwg references
   'v1':int(self.fromv),
   'v2':int(self.tov),
   #x1':self.fromvx,
   #x2':self.tovx,
