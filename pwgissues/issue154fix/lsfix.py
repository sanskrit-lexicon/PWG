# coding=utf-8
""" lsfix_parm.py
    Revision 1: escape periods in regex (lscode1)
    Revision 2: skip_starts
    Revision 3: skip1_starts
    Revision 4: change call sequence in 
                get_regexes_all, get_REGEXes_standard
"""
...
def get_regexes_all(targetparms):
...
def get_REGEXes_standard(targetparms):
 lscode = targetparms['lscode']
 lscode1 = lscode.replace('.','[.]')
...
def get_instances_all(entries, regexes_all,regexes_standard,skip_starts,
                      skip1_starts):
  ...
  for iline,line in enumerate(entry.datalines):
   for reg in regs:
    for m in re.finditer(reg,line):
     ...
     if entry.linenum1 + iline + 1 == 110231:
      pass # print('dbg:',instance.matchstr)
     if skip_instance(instance.matchstr,skip_starts):
      # don't include this instance
      pass
     elif skip_instance(instance.matchstr,skip1_starts):
      # also, don't include this instance
      pass
     else:
...
def init_skip_starts(targetobj):
 ans = []
 #print('init_skip_starts', targetobj)
 if not ('skip' in targetobj):
  return ans
 skips = targetobj['skip'] # array of string
 for skip in skips:
...
def init_skip1_starts(targetobj):
 ans = []
 if 'skip1' in targetobj:
  ans = targetobj['skip1']
...
if __name__=="__main__":
 ...
 regexes_all = get_regexes_all(targetparms)

 REGEXes_standard = get_REGEXes_standard(targetparms)
