# coding=utf-8
""" generate_random_nir.py for nirukta
"""
from __future__ import print_function
import sys, re,codecs
import digentry
from make_js_index_naigh import *
...
def get_dict_key(m,dictcode):
 if True: # dictcode in ('pwg2a','pwg2b'):
  parm1=m.group(1)  
  parm2=m.group(2)
  try:
   key1 = int(parm1)
...
def get_dict_regex(dictcode):
 d = {
  ...
  'pwkvn2b':r'<ls>NAIGH. ([0-9]+),([0-9]+)',
  'sch2a':r'<ls>Nir. ([0-9]+),([0-9]+)',
  'sch2b':r'<ls>Nigh. ([0-9]+),([0-9]+)',
  'mw2a':r'<ls>Nir. ([ivxl]+), ([0-9]+)',
  'mw2b':r'<ls>Naigh. ([ivxl]+), ([0-9]+)',
