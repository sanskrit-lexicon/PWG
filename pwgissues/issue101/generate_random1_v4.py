# coding=utf-8
""" generate_random1_v4.py
"""
from __future__ import print_function
import sys, re,codecs
import digentry
from make_js_index_v4 import *
...
def get_dict_key_mwv4(m):
 # example: <ls>Sāh. vii, 10 -> key: (7, 10)
 p = roman_to_integer(m.group(1))
 k = int(m.group(2))
 key = (p,k)
 #print('%s -> key: %s' % (m.group(0),key))
 #exit(1)
 return key
...
def set_pagerec_key_mwv4(pagerecs):
 # key is a 2-tuple of ints (paricCeda,)
 # In python, n-tuples of ints have the <= relation
 for rec in pagerecs:
...
def get_dict_regex(dictcode):
 d = {
  'mwv4':r'<ls>Sāh. ([ivx]+), *([0-9]+)',   # paricCeda,kArikA
