...
def gather_ab(entry):
 ...
 #allparts = re.split('(<ab.*?</ab>)',entrydata)
 allparts = re.split('(<ab[^>]*>[^<]*?</ab>)',entrydata)
 return allparts,[]
...
def first_diff(e1,e2):
 ...
 for i in range(n):
  if (i < n1) and (i < n2):
   x1 = a1[i]
   x2 = a2[i]
   if x1.startswith('<ab') and x2.startswith('<ab') and x1 != x2:
    if (x1 == '<ab>u.</ab>') and (x2 in
                  ('<ab n="und">u.</ab>','<ab n="unten">u.</ab>')):
     pass  # consider these the same
    elif (x1 == '<ab>d.</ab>') and (x2 in 
      ('<ab n="dem">d.</ab>', '<ab n="die">d.</ab>',
       '<ab n="der">d.</ab>', '<ab n="des">d.</ab>', '<ab n="das">d.</ab>')):
     pass
    else:
     ifirst = i
     break
   elif (x1.startswith('<ab')) and (not x2.startswith('<ab')):
    ifirst = i
    break
   elif (not x1.startswith('<ab')) and (x2.startswith('<ab')):
    ifirst = i
    break
  elif (i < n1) and a1[i].startswith('<ab'):
   ifirst = i
   break
  elif (i < n2) and a2[i].startswith('<ab'):
   ifirst = i
   break
 if ifirst == None:
  return []
