...
def get_regex_split():
 tags3 = ['<div n=.*?>', '<info n=.*?/>']
 tags2 = ('F','VN','ab','fr','hom','is','lang','ls', 'lex','span')
...
def mark_ab_one(line,abpattern,dbg):
...
def test(abpattern):
 #line = 'Interj. '
 #newline = re.sub(abpattern,r"<ab>\1</ab>", line)
 line = '1. {#a#}¦ Interj. {#a apehi#} (die beiden Vocale fliessen nicht in einander)'
 newline = mark_ab_one(line,abpattern)
 print(line)
 print(newline)
 
 
def mark_ab(lines,abpattern):
 newlines = []
 metaline = None
 ...
 for iline,line in enumerate(lines):
  ...
  #dbg = iline == 48544
  dbg = False
  newline = mark_ab_one(line,abpattern,dbg)
