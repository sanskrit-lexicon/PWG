# coding=utf-8
""" generate_random.py for TAITTIRĪYABRĀHMAṆA
"""
...
def get_dict_key(m,dictcode):
 a0 = m.group(1)
 if dictcode.startswith('mw'):
  a = roman_to_integer(a0)
 else:
  a = int(a0)
 b = int(m.group(2))
 c = int(m.group(3))
...
def set_pagerec_key(pagerecs,dictcode):
 # key is a 4-tuple of ints 
 # In python, n-tuples of ints have the <= relation
 for rec in pagerecs:
  a = rec.kand
  b = rec.prapath
  c = rec.anuvak
  rec.keymin = (a,b,c,rec.fromv)
  rec.keymax = (a,b,c,rec.tov)
...
def get_dict_regex(dictcode):
 d = {
  'pwg':r'<ls>TBR. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',   # 

  'pw':r'<ls>TBR. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',   # 

  'pwkvn':r'<ls>TBR. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',   # 
 
  'sch':r'<ls>Tbr. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',   # 

  'mw':r'<ls>TBr. ([i]+), *([0-9]+), *([0-9]+), *([0-9]+)',   # 
