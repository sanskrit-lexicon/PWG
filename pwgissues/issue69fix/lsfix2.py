...
def get_altinstances(instances,altregs):
 regcounts = [0 for regex in altregs]
 altinstances = []
 for instance in instances:
  found = False
  for iregex,regex in enumerate(altregs):
   if re.search(regex,instance.matchstr):
    regcounts[iregex] = regcounts[iregex]+1
    found = True
    break
  if not found:
   altinstances.append(instance)
 return regcounts,altinstances

def write_alt_instances(fileout,altinstances,altregcounts,altregs):
 outarr = []
 ...
 for instance in altinstances:
  ...
  outarr.append(out)
 write_lines(fileout,outarr)
 for ireg,regex in enumerate(altregs):
  count = altregcounts[ireg]
  print('%4d %s' %(count,regex))
 totalfound = sum(altregcounts)
 print('%s instances recognized' % totalfound)
 print('%s instances not recognized' % len(altinstances))
...
if __name__=="__main__":
 ...
 if 'altforms' in targetparms:
  filealt = sys.argv[4] # output for alternate forms
  altregs = targetparms['altforms']
  altregcounts,altinstances = get_altinstances(instances_all_select,altregs)
  write_alt_instances(filealt,altinstances,altregcounts,altregs)
 exit(1)
