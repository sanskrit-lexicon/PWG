# coding=utf-8
""" generate_random.py for bhartr index
"""
from __future__ import print_function
import sys, re,codecs
import digentry
from make_js_index_bhart import *
...
def set_pagerec_key(pagerecs,dictcode):
 ...
 for rec in pagerecs:
  ...
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (shataka,fromv)
  rec.keymax = (shataka,tov)
...
def get_dict_regex(dictcode):
 d = {
  'pwg':r'<ls>BHARTṚ. ([0-9]+),([0-9]+)',

  'pw':r'<ls>BHARTṚ. ([0-9]+),([0-9]+)',
  'pwkvn':r'<ls>BHARTṚ. ([0-9]+),([0-9]+)',

  'sch':r'<ls>Bhartṛ. ([0-9]+)',

  'mw':r'<ls>Bhartṛ. ([iv]+), *([0-9]+)',   # 
