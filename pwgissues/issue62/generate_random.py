# coding=utf-8
""" generate_random.py for amara_dlc 
"""
...
def get_dict_key(m,dictcode):
 if dictcode in ('pwg4','pw4','pwkvn4','sch4'):
  parm1=m.group(1)  
  parm2=m.group(2)
  parm3=m.group(3)  
  parm4=m.group(4)
...
def set_pagerec_key_helper(rec,dictcode):
 if dictcode in ('pwg4','pw4','pwkvn4','sch4'):
  ...
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (b,c,s,fromv)
  rec.keymax = (b,c,s,tov)
 elif dictcode in ('pwg3','pw3','pwkvn3','sch3'):
  b = rec.book
  c = rec.chapter
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (b,c,fromv)
  rec.keymax = (b,c,tov)
 else:
...
def get_dict_regex(dictcode):
 d = {
  # 4 numeric paramaters
  'pwg4':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'pw4':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'pwkvn4':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'sch4':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  # 3 numeric parameters
  'pwg3':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
  'pw3':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
  'pwkvn3':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
  'sch3':r'<ls>AK. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
   }
