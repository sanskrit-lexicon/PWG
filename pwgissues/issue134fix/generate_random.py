...
def set_pagerec_key_helper(rec,dictcode):
 if dictcode.endswith('2'):
  ...
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (pancika,fromv)
  rec.keymax = (pancika,tov)
 elif dictcode.endswith('3'):
  pancika = rec.pancika
  adhy = rec.adhy
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (pancika,adhy,fromv)
  rec.keymax = (pancika,adhy,tov)
 else:
