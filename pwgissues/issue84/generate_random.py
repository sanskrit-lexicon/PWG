...
def generate_pagerec_keys(pagerecs):
 ...
 for rec in pagerecs:
  fromkey = rec.fromkey
  tokey = rec.tokey
  assert fromkey[:3] == tokey[:3]
  base = fromkey[:3]
  v1 = rec.fromkey[3]
  v2 = rec.tokey[3]
  for v in range(v1,v2+1):
   key = (base[0],base[1],base[2],v)
   keys.append(key)
...
pwgregex_raw = r'<ls>ŚAT\. BR\. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)'
...
def init_verseentries(entries):
 ...
 for entry in entries:
  ...
  for m in re.finditer(pwgregex,text):
   versekey = (int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4)))
