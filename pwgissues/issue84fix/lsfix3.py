...
class Instance:
 def __init__(self,m,entry,iline,regexes_standard,nparm):
  ...
  for regex in regexes_standard:
   m = re.search(regex,self.matchstr)
   if m != None :
    self.status = True
    self.parms = []
    for i in range(nparm):
     self.parms.append(m.group(i+1))
    if False:
     print(f'Instance: {self.matchstr} {self.parms}')
     exit(1)
    break
  self.expansions = []
...
def write_instances(fileout,instances):
 ...
 for instance in instances:
  ...
  #out = '%s\t%s\t%s\t%s\t%s\t%s' % (status,nparm,lnum,k1,instance.matchstr,parms1)
  out = '%s\t%s\t%s\t%s' % (lnum,k1,instance.matchstr,parms1)
  if status != True:
   continue
  outarr.append(out)
  stat1 = (status,nparm)
...
if __name__=="__main__":
 ...
 for nparm in nparms:
  ...
  if False: # debug
   print(f'nparm = {nparm}')
   for r in regexes_standard[nparm]:
    print(f' regex = {r}')
