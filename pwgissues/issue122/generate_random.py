...
def init_verseentries(entries,regex):
 # allow roman
 d = {}
 n = 0
 for entry in entries:
  ...
  for m in re.finditer(regex,text):
   adhy = parse_adhy(m.group(1))
   if adhy == None:
    print('adhy error:',m.group(0))
...
def get_dict_regex(dictcode):
 d = {
  'pwg':r'<ls>MĀRK. P. ([0-9]+),([0-9]+)',
  'pw':r'<ls>MĀRK. P. ([0-9]+),([0-9]+)',
  'pwkvn':r'<ls>MĀRK. P. ([0-9]+),([0-9]+)',
  'sch':r'<ls>Mārk. P. ([0-9]+),([0-9]+)',
  'mw':r'<ls>MārkP. ([ivxclm]+), *([0-9]+)',
  # for anomaly at adhy 16,17,18
  'pwg118':r'<ls>MĀRK. P. (118),([0-9]+)',
  'pwg116':r'<ls>MĀRK. P. (116),([0-9]+)',
   }
...
if __name__=="__main__":
 ...
 verseentries = init_verseentries(entries,regex)
