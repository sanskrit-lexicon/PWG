...
parm_vol = r'^(I|II|III)$'
parm_page = r'^([0-9]+)$'
parm_adhy = r'^([khgṅcjñṭḍṇtdnpbmyrlvśṣs]+|avy|coloph)[.]$'
parm_fromv = r'^([0-9]+)([b])?$'
parm_tov = r'^([0-9]+)([a])?$'
parm_hw = r'^([aAiIuUfxFXeEoOMHkKgGNcCjJYwWqQRtTdDnpPbBmyrlvSzsh]*)$'
...
""" 
 def __init__(self,line,iline,filevol):
  ...
  if m_adhy == None:
   ...
   return
  # check hw
  m_hw = re.search(parm_hw,raw_hw)
  if m_hw == None:
   self.status = False
   self.status_message = 'Unexpected hw: %s' % raw_hw
   return
 ...
 def todict(self):
  ...
  e = {
   ...
   'x1':self.fromvx, 'x2':self.tovx, 'vp':self.vpstr, 'hw':self.hw
  }
  return e
