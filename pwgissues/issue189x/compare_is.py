...
def unused_get_difflines(entries1,entries2):
 ans = []
...
def unused_gather_ab(entry):
 ...
 allparts = []
 for m in re.finditer(r'<is(.*?)>(.*?)</is>',entrydata):
  abbrev = m.group(2)
  tip = m.group(2)
  elt = m.group(0)
  allpart = (abbrev,tip,elt)
  allparts.append(allpart)
 return allparts

def unused_first_diff(e1,e2):
 a1 = gather_ab(e1)  # iabs not used
 a2 = gather_ab(e2)
 n1 = len(a1)
 n2 = len(a2)
 n = max(n1,n2)
 ...
 for i in range(n):
  if (i < n1) and (i < n2):
   x1 = a1[i][0]  # match on abbrev only
   x2 = a2[i][0]
   if  x1 != x2:
    ifirst = i
    break
...
def first_diff(e1,e2):
 ...
 for i in range(n):
  if (i < n1) and (i < n2):
   ...
   x2 = a2[i]
   if x1.startswith('<is'):
    m = re.search('<is([^>]*)>([^<]*?)</is>',x1)
    y1 = m.group(2)  
   if x2.startswith('<is'):
    m = re.search('<is([^>]*)>([^<]*?)</is>',x2)
    y2 = m.group(2)  
   if x1.startswith('<is') and x2.startswith('<is') and y1 != y2:
    ifirst = i
    break
   elif (x1.startswith('<is')) and (not x2.startswith('<is')):
    ifirst = i
    break
   elif (not x1.startswith('<is')) and (x2.startswith('<is')):
    ifirst = i
    break
  elif (i < n1) and a1[i].startswith('<is'):
   ifirst = i
   break
  elif (i < n2) and a2[i].startswith('<is'):
   ifirst = i
   break
 if ifirst == None:
  return []
