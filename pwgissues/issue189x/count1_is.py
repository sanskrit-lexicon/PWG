""" coun1t_is.py
 Find occurrence of all instances of `<is.*?</is>
...
if __name__=="__main__":
 ...
 #regex = re.compile(r'<is(.*?)>(.*?)</is>')
 regex = r'<is(.*?)>(.*?)</is>'

 keysd = gather(lines,regex)  # dictionary with counts
 outarr,ntot = get_outarr(keysd)
 write_lines(fileout,outarr)
 print(f'{ntot} = total number of {regex}')
