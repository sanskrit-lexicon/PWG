...
def get_dict_key(m,dictcode):
 if dictcode.endswith('2'):
  return get_dict_key_2(m)
 if dictcode.endswith('1'):
  return get_dict_key_1(m)
 if dictcode.endswith('3'):
  return get_dict_key_3(m)
 print('get_dict_key ERROR:',dictcode)
 exit(1)

def get_dict_key_1(m):
 verse = int(m.group(1))
 key = (verse,)
 return key

def get_dict_key_2(m):
 ipage = int(m.group(1))
 linenum = int(m.group(2))
 key = (ipage,linenum)
 return key

def get_dict_key_3(m):
 anka_roman = m.group(1)
 anka = roman_to_integer(anka_roman)
 verse = int(m.group(2))
 key = (anka,verse)
 return key
...
def set_pagerec_key_2(pagerecs):
 # key is a 2-tuple of ints (ipage,lilnenum)
 # In python, n-tuples of ints have the <= relation
 for rec in pagerecs:
  rec.keymin = (rec.ipage,1)
  rec.keymax = (rec.ipage,999)

def set_pagerec_key_3(pagerecs):
 # key is a 2-tuple of ints (ipage,linenum)
 # In python, n-tuples of ints have the <= relation
 for rec in pagerecs:
  rec.keymin = (rec.ipage,1)
  rec.keymax = (rec.ipage,999)

def set_pagerec_key_1(pagerecs):
 # key is a 1-tuple of ints (verse,)
 # In python, n-tuples of ints have the <= relation
 for rec in pagerecs:
...
def set_pagerec_key(pagerecs,dictcode):
 if dictcode.endswith('2'):
  set_pagerec_key_2(pagerecs)
 elif dictcode.endswith('1'):
  set_pagerec_key_1(pagerecs)
 elif dictcode.endswith('3'):
  set_pagerec_key_3(pagerecs)
 else:
  print('set_pagerec_key ERROR',dictcode)
  exit(1)
...
def get_dict_regex(dictcode):
 d = {
  'mw1':r'<ls>Śak. ([0-9]+)[^0-9,].*?</ls>',  # verse
  'mw2':r'<ls>Śak. ([0-9]+), *([0-9]+).*?</ls>',   # ipage,linenum
  'mw3':r'<ls>Śak. ([iv]+), ([0-9]+).*?</ls>',

   }
 if dictcode not in d:
  print('get_dict_regex Error "%s"' % dictcode)
  exit(1)
...
def write_verseentries(fileout,d):
 keys0 = d.keys()
 keys = sorted(keys0)
 outarr = []
 for key in keys:
  # key is a tuple
  #keyout = "%s,%s" % key
  keystr = [str(x) for x in key]
  keyout = ','.join(keystr)
  keyvals0 = d[key]
  #(entries0,ls) = d[key]
  keyvals = sorted(keyvals0,key = lambda K: float(K[0].metad['L']))
  for keyval in keyvals:
   entry,ls = keyval
   meta = entry.metaline
   meta = re.sub('<k2>.*$','',meta)
   dataout = (keyout,meta,ls)
   out = '\t'.join(dataout)
   outarr.append(out)
...
if __name__=="__main__":
 ...
 verseentries = init_verseentries(entries, code)
 write_verseentries(fileout,verseentries)
