...
class Change:
 def __init__(self,line):
  ...
  if len(parts) != 2:
   ...
   exit(1)
  self.oldword = parts[0].strip()
  self.newword = parts[1].strip()
...
regexsplitraw = r'(<(?P<tag>[^ >]+).*?>.*?</(?P=tag)>)||({#.*?#})|(<div.*?>)|(¦)|([A-Za-z0-9äöüÄÖÜáéèÁÉÈ]+)'

regexsplit = re.compile(regexsplitraw)

word_regex_raw = '[A-Za-z0-9äöüÄÖÜáéèÁÉÈ]+'
word_regex = re.compile(word_regex_raw)
...
def update_recs(entries,recs):
 ...
 for rec in recs:
  w = rec.oldword
  if w in drec:
   print('update_recs error 1',rec.linein)
   exit(1)
  drec[w] = rec
 #
 for ientry,e in enumerate(entries):
