...
def get_dict_key(m,dictcode):
 if dictcode in ('pwg3','pw3','pwkvn3','sch3','mw3'):
  parm1=m.group(1)  
  parm2=m.group(2)
  parm3=m.group(3)  
  #parm4=m.group(4)
...
def set_pagerec_key_helper(rec,dictcode):
 if dictcode in ('pwg3','pw3','pwkvn3','sch3','mw3'):
  ...
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (r,a,fromv)
  rec.keymax = (r,a,tov)
 elif dictcode in ('pwg1',):
  rec.keymin = (1,)
  rec.keymax = (371,)
 else:
...
def get_dict_regex(dictcode):
 d = {
  # 
  'pwg3':r'<ls>PAÑCAR. ([0-9]+),([0-9]+),([0-9]+)',
  'pwg1':r'<ls>PAÑCAR. S. ([0-9]+)',
  'pw3':r'<ls>PAÑCAR. ([0-9]+),([0-9]+),([0-9]+)',
  'pwkvn3':r'<ls>PAÑCAR. ([0-9]+),([0-9]+),([0-9]+)',
  'sch3':r'<ls>Pañcar. ([0-9]+),([0-9]+),([0-9]+)',
  'mw3':r'<ls>Pañcar. ([ivx]+), *([0-9]+), *([0-9]+)',
   }
 if dictcode not in d:
