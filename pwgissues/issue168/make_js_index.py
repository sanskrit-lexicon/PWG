# coding=utf-8
""" make_js_index.py for pancatantra
"""
...
parm_ipage = r'^([0-9]+)$'
parm_vpstr_format = '%02d'  # note 02d

class Pagerec(object):
 """
Format 
 page	tantra	fromv	tov ipage
29	1	1	5   xxx
...
def check1_tantra(pagerecs):
 ...
 for irec,rec in enumerate(pagerecs):
  ...
  if irec == 0:
   ...
   if rec.tantra != 0:
    print('check1_tantra: first tantra not 0.')
  ...
  if prev.tantra == rec.tantra:
   ...
   print('lnum=%s, line=%s' % (lnum,line))
   #print('prev=',prev)
   print(f'prev.iline = {prev.iline}')
   print(f'prev.line = {prev.line}')
   exit(1)
  # reset prev
  prev = rec
