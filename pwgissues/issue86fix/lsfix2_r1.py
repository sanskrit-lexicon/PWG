# coding=utf-8
""" lsfix2_r1.py
  2 nparm values  1st parm is either
  a) digit sequence  [0-9]+
  b) Upper case roman [IV]+
"""
...
class REGEX1:
 def __init__(self,n,S):
  ...
  self.W = reg_help(self.n)
  self.regex = '^' + self.W + self.S
  
 def unused_reg_help(self,n):
  a = []
  for i in range(n):
   a1 = '([0-9]+|[IV]+)'  # digits or roman # a1 = '([0-9]+)'
   a.append(a1)
  ans = ','.join(a)
  return ans
 ...
 def __init__(self,m,entry,iline,regexes_standard,nparm):
  ...
  for regex in regexes_standard:
   if re.search(regex,self.matchstr):
    ...
    break
  if self.status == None:
   status = pancat_match(self.matchstr)
   if status == True:
    self.status = True
  self.expansions = []

def pancat_match(matchstr):
 ls = 'PAÑCAT.'
 regexes = [
  '<ls>PAÑCAT. ed. orn. [0-9]+,[0-9]+\.?</ls>',
  '<ls>PAÑCAT. ed. orn. [IV]+,[0-9]+\.?</ls>',
  '<ls n="PAÑCAT.">ed. orn. [0-9]+,[0-9]+\.?</ls>',
  '<ls n="PAÑCAT.">ed. orn. [IV]+,[0-9]+\.?</ls>',
  '<ls n="PAÑCAT. ed. orn.">[0-9IV]+,[0-9]+\.?</ls>',
  '<ls>PAÑCAT. Pr. [0-9]+\.?</ls>',
  '<ls n="PAÑCAT.">Pr. [0-9]+\.?</ls>',
  '<ls>PAÑCAT. ed. Bomb\..*?</ls>',
  '<ls n="PAÑCAT. ed. Bomb\.">.*?</ls>',
  '<ls>PAÑCAT. ed. orn\..*?</ls>',
  '<ls>ed. orn\..*?</ls>',
  ]
 for regex in regexes:
  if re.search(regex,matchstr):
   return True
 return None
...
if __name__=="__main__":
 ...
 for nparm in nparms:
  ...
  if False: # True:
   print("regexes_standard[%s]" % nparm)
   for r in a:
    print(r)
