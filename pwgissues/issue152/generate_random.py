...
def unused_keyconvert_RN_mw_2_int(x):
 vals = ('Introd.','i','ii','iii','iv')
 for ival,val in enumerate(vals):
  if x == val:
   return ival
 print('keyconvert_RN_mw_2_int ERROR: x=',x)
 exit(1)

def get_dict_key(m,dictcode):
 if dictcode in ('pwgRN','pwRN','pwkvnRN','schRN'):
  parm1=m.group(1)  # 
  parm2=m.group(2)
 ...
 if dictcode in ('mwRN',):
  ...
  key = (key1,key2)
  return key
 if dictcode in ('pwgPN','pwPN','pwkvnPN','schPN'):
  parm1=m.group(1)  # 
  parm2=m.group(2)
  key1 = 0
  key2 = int(parm2)
 ...
 if dictcode in ('mwPN',):
  parm1=m.group(1)  #
  parm2=m.group(2)
  key1 = 0 # the app for hitopadesha uses '0' for prastAva
  key2 = int(parm2)
  key = (key1,key2)
  return key
 
 if dictcode in ('pwgNN','pwNN','pwkvnNN','schNN','mwNN'):
  parm1=m.group(1)  # 
  parm2=m.group(2)
  key1 = int(parm1)
  key2 = int(parm2)
...
def set_pagerec_key_helper(rec,dictcode):
 if dictcode in ('pwgRN','pwRN','pwkvnRN','mwRN','schRN'):
  a = rec.tantra  # 0,1,2,3,4
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (a,fromv)
  rec.keymax = (a,tov)
 elif dictcode in ('pwgPN','pwPN','pwkvnPN','mwPN','schPN'):
  a = rec.tantra  # 0
  fromv = rec.fromv
  tov = rec.tov
  rec.keymin = (a,fromv)
  rec.keymax = (a,tov)
 elif dictcode in ('pwgNN','pwNN','pwkvnNN','schNN','mwNN'):
  ipage = rec.ipage
  minlnum = 1
  maxlnum = 40 # arbitrary.
  tov = rec.tov
  rec.keymin = (ipage,minlnum)
  rec.keymax = (ipage,maxlnum)
 else:
...
def get_dict_regex(dictcode):
 d = {
  # tantra, verse
  'pwgRN':r'<ls>HIT. (I|II|III|IV),([0-9]+)',
  'pwRN':r'<ls>HIT. (I|II|III|IV),([0-9]+)',
  'pwkvnRN':r'<ls>HIT. (|I|II|III|IV),([0-9]+)',
  'mwRN':r'<ls>Hit. (i|ii|iii|iv), *([0-9]+)',
  'schRN':r'<ls>Hit. (I|II|III|IV),([0-9]+)',
  # prastAva, verse
  'pwgPN':r'<ls>HIT. (Pr.) ([0-9]+)',
  'pwPN':r'<ls>HIT. (Pr.) ([0-9]+)',
  'pwkvnPN':r'<ls>HIT. (Pr.) ([0-9]+)',
  'schPN':r'<ls>Hit. (Pr.) ([0-9]+)',
  'mwPN':r'<ls>Hit. (Introd.) *([0-9]+)',
  # ipage, line-number
  'pwgNN':r'<ls>HIT. ([0-9]+),([0-9]+)',
  'pwNN':r'<ls>HIT. ([0-9]+),([0-9]+)',
  'pwkvnNN':r'<ls>HIT. ([0-9]+),([0-9]+)',
  'schNN':r'<ls>Hit. ([0-9]+),([0-9]+)',
  'mwNN':r'<ls>Hit. ([0-9]+), *([0-9]+)',
