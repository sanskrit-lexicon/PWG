...
def write_changes(fileout,changes):
 ...
 for c in changes:
  ...
  for rep in c.replacements:
   ...
   outarr.append('; prevls:%s' % rep.ls0)
...
class Replacement:
 def __init__(self,old,new,ls0):
  self.old = old
  self.new = new
  self.count = 0  # number of inputs
  self.countfound = 0  # number of usages in entries (see get_newline)
  self.ls0 = ls0  # previous ls
...
def get_newline(line,drec):
 ...
 for ls in lsarr:
  ...
  rep.countfound = rep.countfound + 1
 if replacements == []:
  return line,replacements
 # generate newline
 newline = line
 for rep in replacements:
  # modify newline only when rep.count == 1
  # if this count > 1, then there i
  old = rep.old
  new = rep.new
