# coding=utf-8
""" make_js_index_kanda.py for Hemacandra.Anekarthasangraha.1807.pdf
"""
...
""" 
 def __init__(self,line,iline):
  ...
  if m_kanda == None:
   self.status = False
   self.status_message = 'Unexpected kanda: %s' % raw_adhy
   return
  # check adhy
...
def check1_kanda_adhy_errmsg(prev,rec,lnum):
 print('check1_kanda_adhy error')
 print('prev lnum=%s: line=%s' % (lnum-1,prev.line))
 print(' rec lnum=%s: line=%s' %(lnum,rec.line))
 exit(1)
  
def check1_kanda_adhy(pagerecs):
 prev = None
 for irec,rec in enumerate(pagerecs):
  lnum = rec.iline + 1
  line = rec.line
  if irec == 0:
   # first record has kanda = 1 and adhy = 1
   if not (rec.kanda,rec.adhy) == (1,1):
    print('check1_kanda_adhy: first kanda,adhy not 1,1.')
    print('lnum=%s, line=%s' % (lnum,line))
    exit(1)
   prev = rec
   continue
  if prev.kanda != rec.kanda:
   if rec.kanda != (prev.kanda + 1):
    check1_kanda_adhy_errmsg(prev,rec,lnum)
   if (rec.adhy != 1):
    check1_kanda_adhy_errmsg(prev,rec,lnum)
   prev = rec
   continue
  # prev kanda == rec.kanda       
  if prev.adhy == rec.adhy:
   pass # no problem
  elif (prev.adhy + 1) == rec.adhy:
   pass
  ...
  # reset prev
  prev = rec
 print('pagerecs passes check1_kanda_adhy ')
