# coding=utf-8
""" generate_random.py for amara_col
"""
...
def get_dict_key(m,dictcode):
 if dictcode in ('pwg4a','pwg4b','pwg4c'):
  parm1=m.group(1)  
  parm2=m.group(2)
  parm3=m.group(3)  
  parm4=m.group(4)
  ...
  return key
 if dictcode in ('pwg3a','pwg3b','pwg3c'):
  parm1=m.group(1)  
  parm2=m.group(2)
  parm3=m.group(3)  
  key1 = int(parm1)
...
def set_pagerec_key_helper(rec,dictcode):
 ...
 elif dictcode in ('pwg3a','pwg3b','pwg3c'):
  b = rec.book
  c = rec.chapter
  fromv = rec.fromv
  tov = rec.tov
...
def get_dict_regex(dictcode):
 d = {
  # 
  'pwg4a':r'<ls>COL. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'pwg3a':r'<ls>COL. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
  'pwg4b':r'<ls>COLEBR. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'pwg3b':r'<ls>COLEBR. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
  'pwg4c':r'<ls>AK. ed. COLEBR. ([0-9]+),([0-9]+),([0-9]+),([0-9]+)',
  'pwg3c':r'<ls>AK. ed. COLEBR. ([0-9]+),([0-9]+),([0-9]+)[^0-9,]',
   }
 if dictcode not in d:
