...
def make_number_change(lsold,lsabbr,n1):
 # 1)
 m = re.search(r'^<ls>([0-9]+,) ([0-9]+[.]?)</ls>$',lsold)
 if m:
  a1 = m.group(1)
  a2 = m.group(2)
  lsnew = '<ls n="%s">%s %s</ls>' %(lsabbr,a1,a2)
  return lsnew
 # 2)
 ...
 if m:
  ...
  a2 = m.group(1)
  a3 = m.group(2)
  lsnew = '<ls n="%s n=%s,">%s %s</ls>' %(lsabbr,a1,a2,a3)
  return lsnew
 # 5)
 ...
 if m:
  ...
  a2 = m.group(2)
  a3 = m.group(3)
  lsnew = '<ls n="%s">%s %s</ls> <ls>%s' %(lsabbr,a1,a2,a3)
  return lsnew
 # 6)
...
def generate_changes(entries,lsabbr):
 # works for LS whose citations take 2 parameters
 ...
 ndefer = 0
 regexes = [r'<ls>%s ([0-9]+), ([0-9]+)' %lsabbr,
            r'<ls n="%s">([0-9]+), ([0-9]+)' %lsabbr,
            r'<ls n="%s ([0-9]+),">([0-9]+)' %lsabbr,
  ]
  ...
  for iline,line in enumerate(entry.datalines):
   ...
   for m in re.finditer(r'<ls(.*?)>(.*?)</ls>',line):
    ...
    if isnumeric:
     ...
     lsnew = make_number_change(lsold,lsabbr,n1)
