...
from remark_ls import remark_pwg
# (lselt,abbrev,nparmabbrev)
...
class LSCase(object):
 def __init__(self,ls,abbrev,metaline,iline,line,nparmabbrev,lselt):
  self.lselt = lselt # <ls(.*?)>(.*?)</ls>
  self.ls = ls  # ls element (flattened)
  self.nparmabbrev = nparmabbrev  # standard number of parameters
  self.abbrev = abbrev
  self.metaline = metaline
  self.iline = iline
  self.line = line
...
def init_lscases(lines,tipd,abbrev,nparmabbrev,zeroparmerr):
 regex_ls0 = r'<ls([^>]*)>([^<]*)</ls>'
 regex_ls = re.compile(regex_ls0)
 if nparmabbrev == 2:
  # We allow the period after final number to be optional: [.]?
  regex_regular0 = '^%s [0-9]+, [0-9]+[.]?$' % abbrev
 else:
  regex_regular0 = '^XXX$' # never matches. So generates change
  print('init_lscases not handling abbreviation parameters',2)
  #exit(1)
 regex_regular = re.compile(regex_regular0)
 #
 ...
 for iline,line in enumerate(lines):
  ...
  if line.startswith('[Page'):
   page = line
   continue
  for m in re.finditer(regex_ls,line):
   lselt = m.group(0)
   attrib = m.group(1)
   elt = m.group(2)
   ...
   if tip.abbrev != abbrev:
    continue
   # only display cases with non-standard parameters
   if re.search(regex_regular,elt):
    continue
   # if zeroparmerr is False, consider <ls>abbrev</ls> as correct (0-parms)
   if (not zeroparmerr) and (elt == abbrev):
    continue
   # found a match
   lscase = LSCase(elt,abbrev,metaline,iline,line,nparmabbrev,lselt)
   entry.append(lscase)
...
def write_lsentries(fileout,lsentries,abbrev):
 ...
 for lscases in lsentries:
  ...
  outarr.append('; %s {%s %s}' %(x,abbrev,n))
  #f.write(';-----------------------------------------------------------')
  
  for lscase in lscases:
   lselt_new,status = remark_pwg(lscase.lselt,lscase.abbrev,lscase.nparmabbrev)
   outarr.append('; %s  STATUS=%s' %(lscase.ls,status))
   outarr.append('%s old %s' %(lscase.iline+1,lscase.line))
   newline = lscase.line.replace(lscase.lselt,lselt_new)
   outarr.append(';')
   outarr.append('%s new %s' %(lscase.iline+1,newline))
  for out in outarr:
   f.write(out+'\n')
...
if __name__=="__main__":
 ...
 tempoptions  = sys.argv[2].split(',')
 assert len(tempoptions) == 2
 # standard number of numerical parameters for the abbrev in pwg.
 nparmabbrev = int(tempoptions[0])
 if tempoptions[1] == 'False':
  zeroparmerr = False
 else:
  zeroparmerr = True   # 0 parms counted as an error and written to changes
 ...
 with codecs.open(filein,"r","utf-8") as f:
  lines = [x.rstrip('\r\n') for x in f]
 lsentries = init_lscases(lines,tipd,abbrev,nparmabbrev,zeroparmerr) 
