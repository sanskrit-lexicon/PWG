...
def count_tips(lines,tipd,numbertip,unknowntip,fileunk):
 lsunknowns = []
 ...
 for iline,line in enumerate(lines):
  ...
  for m in re.finditer(r'<ls([^>]*)>([^<]*)</ls>',line):
   ...
   if re.search(r'^[0-9]',elt): # number
    ...
    tip = unknowntip
    lsunknowns.append((metaline,iline,m.group(0)))
   else:
    tiplist = tipd[elt[0]]
 ...
 #return lsentries
 write_lsunknowns(fileunk,lsunknowns)
...
if __name__=="__main__":
 ...
 fileunk = sys.argv[4] # instances of unknown tooltips
 ...
 with codecs.open(filein,"r","utf-8") as f:
  lines = [x.rstrip('\r\n') for x in f]
 count_tips(lines,tipd,numbertip,unknowntip,fileunk) # also, updates tip.changes
