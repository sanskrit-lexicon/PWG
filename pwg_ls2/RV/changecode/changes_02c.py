...
def refactor_3(ls,abbrev,lsprev):
 """ ls == one or more of X,Y,Z. 
 """
 # parse lsprev
 ...
 while True:
  rest = rest.strip()  # remove spaces at end
  m = re.search(r'^([0-9]+)[.] *(.*)$',rest)
  if m == None:
   break
  part = '<ls n="%s %s, %s,">%s.</ls>' %(abbrev,x1,x2,m.group(1))
  parts.append(part)
  rest = m.group(2)
 if len(parts) == 0:
  return "<ls>%s</ls>" %ls
 ...
 newls = '%s %s' %(new1,new2)
 return newls
def refactor_2(ls,abbrev,lsprev):
 """ ls == one or more of X,Y,Z. 
 """
 # parse lsprev
 m = re.search(r'%s ([0-9]+)[ ,]+([0-9]+)[ ,]+([0-9]+)[.]?$' %abbrev,lsprev)
...
def write_lscases_num(fileout,cases,abbrev):
 ...
 for lscase in cases:
  ...
  newlsfull = refactor_3(oldls,abbrev,prevls)
