...
def init_changes(lines,tipd,abbrev):
 ...
 for iline,line in enumerate(lines):
  ...
  if metaline == None:
   continue
  # does abbrev appear in line NOT preceded by <ls> or <ls n="
  # add underscore ('_' does not otherwise occur in pwg.txt) 
  newline = re.sub(r'([^>"])%s'% abbrev, r'_\1%s'%abbrev,line)
