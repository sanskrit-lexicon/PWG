...
from ak_index_verses_set import *
...
def write_doneparms(fileout,d,option):
 #keys = d.keys()
 ...
 for parmtuple in keys1:
  ...
  for instance,metaline,parmstr,lnum,line in rec:
   if option == '1':
    pass
   elif option == '2':
    if parmstr.strip().startswith('3, 4,'):
     continue
    if not instance.startswith('<ls n="AK.'):
     continue
   if lnum in lnumsd:
    continue # skip change of line a second time
   lnumsd[lnum] = True
   outarr = []
   outarr.append(r'; ----------------------------------------------------')
   meta = re.sub(r'<k2>.*$','',metaline)
   parmtuplestr = str(parmtuple)
   outarr.append('; %s %s' %(meta,parmtuplestr))
   outarr.append('; %s' %instance)
   old = line
   new = line.replace(instance,'**' + instance)
 ...
 with codecs.open(fileout,"w","utf-8") as f:
  for outarr in outrecs:
   for out in outarr:
    f.write(out+'\n')
 print(len(outrecs)-1,"records written to",fileout)
