...
def count_tips(lines,tipd,numbertip,unknowntip,filenum):
 lsnums = []
 ...
 #return lsentries
 write_lsnums(filenum,lsnums)

def write_lsnums(fileout,lsnums):
 # fileout = "lsnums.txt"
 with codecs.open(fileout,"w","utf-8") as f:
  for temp in lsnums:
   metaline,iline,lsunknown = temp
   meta = re.sub(r'<k2>.*$','',metaline)
   meta1 = meta.ljust(30)
   out = '%s\t%s\t%s' %(iline,meta1,lsunknown)
   f.write(out+'\n')
 print(len(lsnums),"lsnums written to",fileout)
...
if __name__=="__main__":
 ...
 filenum = sys.argv[4] # instances of unknown tooltips
 ...
 with codecs.open(filein,"r","utf-8") as f:
  lines = [x.rstrip('\r\n') for x in f]
 count_tips(lines,tipd,numbertip,unknowntip,filenum) # also, updates tip.changes
