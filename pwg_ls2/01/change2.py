...
def change4(line):
 reason = '" Sch. (not in ls)"'
 # exclude cases where pattern appears within <ls>x</ls>
 ...
 for part in parts:
  if part == None:
   ...
   newpart = part.replace(' Sch. ',' <ls>Sch.</ls> ')
   newparts.append(newpart)
 newline = ''.join(newparts)
 return reason,newline

def change5(line):
 reason = '"Sch. " at beg of line'
...
def change7(line):
 reason = '"(Sch.: "'
 newline = line.replace('(Sch.:', '(<ls>Sch.</ls>:')
 return reason,newline
